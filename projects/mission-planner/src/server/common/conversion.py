...
def convert_gps_to_utm(latitude, longitude):
    ...
    utm_e_km = E0 + K0 * A * (
        eta_prime
        + sum(
            ALPHA[j] * cos(2 * j * xi_prime) * sinh(2 * j * eta_prime)
            for j in range(1, 4)
        )
    )
    utm_n_km = N0 + K0 * A * (
        xi_prime
        + sum(
            ALPHA[j] * sin(2 * j * xi_prime) * cosh(2 * j * eta_prime)
            for j in range(1, 4)
        )
    )
...
def convert_utm_to_gps(easting, northing, zone, hemisphere):
    ...
    xi_prime = xi - sum(
        [BETA[j] * sin(2 * j * xi) * cosh(2 * j * eta) for j in range(1, 4)]
    )
    eta_prime = eta - sum(
        [BETA[j] * cos(2 * j * xi) * sinh(2 * j * eta) for j in range(1, 4)]
    )
