...
def scan_area(center_lat, center_lng, altitude, target_area_radius) -> WaypointQueue:
    ...
    scan_radius = calculate_scan_radius(
        altitude, 44, 57
    )  # from v1226-mpz 20MP Lens (12 mm focal)
    logger.debug(f"Scan radius: {scan_radius}")

    # go to center waypoint (with generous slack)
    wpq.push(
        Waypoint(0, "", center_lat, center_lng, altitude, command=SPLINE_WAYPOINT_TYPE)
    )
    count += 1
    ...
    tmp_lat, tmp_lng = convert_utm_to_gps(
        center_we + target_area_radius / 2,
        center_sn - target_area_radius / 2,
        zone,
        hemisphere,
    )
    record.append(
        (center_we + target_area_radius / 2, center_sn - target_area_radius / 2)
    )
    wpq.push(Waypoint(count, "", tmp_lat, tmp_lng, altitude))
    count += 1

    tmp_lat, tmp_lng = convert_utm_to_gps(
        center_we + target_area_radius, center_sn, zone, hemisphere
    )
    ...
    while current_radius >= TURNING_RADIUS:
        ...
        record.append(
            (
                center_we + current_radius * math.cos(current_angle),
                center_sn + current_radius * math.sin(current_angle),
            )
        )
        tmp_lat, tmp_lng = convert_utm_to_gps(
            center_we + current_radius * math.cos(current_angle),
