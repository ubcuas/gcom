...
from server.common.encoders import command_string_to_int
...
def set_home(
    handler: MavlinkHandler, latitude: float, longitude: float, altitude: float
):  # -> int | None:
    ...
    for seq in range(1, len(waypoint_queue) + 1):
        ...
        wp_list.append(
            mavutil.mavlink.MAVLink_mission_item_int_message(
                ...
                float(wp._param3),
                float(wp._param4),
                int(wp._lat * 10000000),
                int(wp._lng * 10000000),
                int(wp._alt),
            )
        )
...
def send_waypoints(handler: MavlinkHandler, wp_list: list) -> bool:
    ...
    for i in range(len(wp_list)):
        msg = handler.wait_for_any_message(
            ["MISSION_REQUEST_INT", "MISSION_REQUEST"], timeout=3.0
        )
        if not msg:
