...
def get_status(status_cache: StatusCache) -> Status:
    ...
    system_time = status_cache.get_message("SYSTEM_TIME") or Object(
    ...
    status_vfr = status_cache.get_message("VFR_HUD") or Object(
        airspeed=0, groundspeed=0, climb=0
    )
    status_sys = status_cache.get_message("SYS_STATUS") or Object(voltage_battery=0)
    status_wpn = status_cache.get_message("MISSION_CURRENT") or Object(
        seq=0, total=0, mission_state=0, mission_mode=0, mission_id=0
    )
    ...
    winddirection = (
        math.degrees(math.atan(status_wind.wind_x / status_wind.wind_y))
        if status_wind.wind_y != 0
        else (0 if status_wind.wind_x > 0 else 180)
    )
    windvelocity = math.sqrt(
        status_wind.wind_x * status_wind.wind_x
        + status_wind.wind_y * status_wind.wind_y
    )
    ...
    for current in range(msg.count):
        ...
        if msg and msg.get_type() != "BAD_DATA":
            ...
            ret.push(
                Waypoint(
                    msg.seq,
                    f"Mission Waypoint {msg.seq}" if msg.seq != 0 else "Home Waypoint",
                    msg.x / 10000000,
                    msg.y / 10000000,
                    msg.z,
            ...
            raise TimeoutError(
                f"Failed to receive MISSION_ITEM_INT for waypoint {current}"
            )
